...
## How React Any Project Works Actually : -
...
Virtual DOM:- 
 • React very fast coz of 'Virtual DOM' of React.
 • React creates an in-memory structure called the virtual DOM which consist of React all components, states, props unlike Real DOM consist of html/documenet's <div>, <h1>, made by Browser.
 • It is Different from the actual browser DOM.
 • It's a lightweight representation where each node stands for a 
component and its attributes.



Reconciliation Process in 'Virtual DOM' :-
 • When React component data changes, Then React firstly updates React's virtual DOM, not Real DOM of brower.

 • Since, React creates two copies of Virtual DOM, In first copy It stores all changes and then then it compares with previous versions/second copy of the 
virtual DOM, and if some changes occures in these two copies in Virtual DOM. Then It finds the specific nodes where where it will only updates the real DOM nodes, The reason react app is fast.
 • It identifies the specific nodes of HTML that need updating after finding change in final Virtual DOM.
...
•React and ReactDOM Libraries :
 ...
 • It's handled by a companion library called 'react-dom' library which is resposible for converting into browser UI; like for converting into android UI, we need React and ReactNative Libraries.
 ...
 • While 'react-dom' library helps build web UIs using React, 'ReactNative' library can be used to craft mobile app UIs





## React Developer Tool Advantages In Using React-Project : -
...
## React Vs Angular vs Vue : -
 ...
 • Libray always written for some specific tasks/things not for entire or complete things.
 • React is a library, while Angular and Vue.js are frameworks.
 • React focuses on dynamic UI only unlike Angular and Vue.js offer comprehensive end to end solution for full App Development including authentication, server-design, network call, routing, error handing etc.  
 ...
 • React doesn't handle/provides inbuilt routing(external React Router), HTTP calls, state management(external Redux), and more.

 •React's Flexibility:
 • React doesn't dictate/bound on tool choices for other app aspects. We can use any other external libraries as per use But Angular and VueJS dictate or bound us to use their inbuilt libraries only.
...
a steep learning curve./takes a long time in learninhg all tools of it.
